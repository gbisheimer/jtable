    ...
    $.extend(true, $.hik.jtable.prototype, {
        ...
        _ajax: function(options) {
            ...
            switch (action) {
                case 'listAction':
                    ...
                    if( self._DBLoaded === true || self.options.actions[action + 'Server'] === true ){
                        return self._ajaxLocalDB( options );
                    }

                    var opts = $.extend(true, {}, options);
                    
                    opts.url = self.options.actions[action + 'Server'];
                    if (opts.urlParams) {
                        // TODO: Sacar los comandos de paginación de la URL
        ...
        _onDBList: function(event, data) {
            ...
            if( self._lastSorting.length > 0 ){
                ...
                self._DB.sort( dynamicSortMultiple(self._lastSorting) );                
            }
            
            self._trigger("onDBList", event, {Records: self._DB});

            // Return data with according to paging options
            return {
                Result: 'OK',
                TotalRecordCount: self._DB.length,
                Records: data.urlParams && data.urlParams.jtStartIndex ? self._DB.slice(Number(data.urlParams.jtStartIndex), Number(data.urlParams.jtStartIndex) + Number(data.urlParams.jtPageSize)) : self._DB
            };
        },
        /**
        ...
        _onDBCreate: function(event, data) {
            ...
            if( !data.data.hasOwnProperty(keyField) ){
                var keys = self._DB.map(function(record){return record.id;});
                keys.push(0);
                record[keyField] = Math.max.apply(null,keys) + 1;
            }

            var success = self._trigger("onDBCreate", event, {data: data.data, record: record, result: result});
        ...
        _findRecordFromKey: function(data){
            ...
            // Gets target record
            var records = self._DB.filter(function(record, index) {
